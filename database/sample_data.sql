...
-- Insert patient records (user_id references from users table)
...
-- Insert medical staff records (user_id references from users table)
...
-- Sample medical records (patient_id from patients table, doctor_id from medical_staff table)
...
-- Sample prescriptions (record_id from medical_records table, medication_id from medications table)
...
-- Sample allergies (patient_id from patients table, recorded_by from medical_staff table)
...
-- Sample patient conditions (patient_id from patients table, condition_id from medical_conditions table, diagnosed_by from medical_staff table)
...
-- Sample surgery (patient_id from patients table, surgeon_id from medical_staff table)
...
-- Sample lab results (patient_id from patients table, ordered_by from medical_staff table)
...
SELECT '=== SAMPLE LOGIN CREDENTIALS ===' as '';
SELECT 'All passwords are: password123' as '';
SELECT '' as '';
SELECT 'PATIENTS:' as 'Account Type', username as 'Username', CONCAT(first_name, ' ', last_name) as 'Full Name' FROM users WHERE user_type_id = 1;
SELECT '' as '';
SELECT 'DOCTORS:' as 'Account Type', username as 'Username', CONCAT(first_name, ' ', last_name) as 'Full Name' FROM users WHERE user_type_id = 2;
SELECT '' as '';
SELECT 'SURGEONS:' as 'Account Type', username as 'Username', CONCAT(first_name, ' ', last_name) as 'Full Name' FROM users WHERE user_type_id = 3;
SELECT '' as '';
SELECT 'Next Steps:' as '';
SELECT '1. Login with any of the above credentials' as '';
SELECT '2. Password for all accounts: password123' as '';
SELECT '3. Test patient dashboard and medical staff features' as '';